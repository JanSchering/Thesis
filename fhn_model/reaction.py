...
def p1(cells:t.Tensor, N:int, gamma:float, k1:float) -> t.Tensor:
  n = cells[0]
  k1_bar = k1 / ((N-1) * (N-2))
  print(n, k1, k1_bar, gamma * k1_bar * n * (n-1) * (N - n))
...
def sigma(cells:t.Tensor, channel:int, N:int, gamma:float, rate_coefficient: float, probability_func: Callable):
  ...
  if channel == 0 or channel == 2:
    #print([f"{prob:.2f}" for prob in reaction_probs])
    cells[0] += t.heaviside(reaction_probs - thresholds, values=t.zeros(num_cells))
  elif channel == 1 or channel == 3:
    cells[0] -= t.heaviside(reaction_probs - thresholds, values=t.zeros(num_cells))
  elif channel == 4:
    cells[1] += t.heaviside(reaction_probs - thresholds, values=t.zeros(num_cells))
  elif channel == 5:
    cells[1] -= t.heaviside(reaction_probs - thresholds, values=t.zeros(num_cells))
  return cells

def rho(grid:t.Tensor, N:int, gamma:float, rate_coefficients: List[float], probability_funcs: List[Callable], num_reaction_channels: int):
  ...
  for channel_idx in range(num_reaction_channels):
    ...
    grid[:, channel_mask] = sigma(cells, channel_idx, N, gamma, rate_coefficient, p_func)
  return grid